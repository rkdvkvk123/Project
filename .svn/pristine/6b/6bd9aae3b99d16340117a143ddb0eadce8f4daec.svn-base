  	...
  	<div class="box1" style="overflow:scroll; width:300px; height:900px;">
		...
		<!-- 게시판 하단의 페이징  -->
		    <c:if test="${pageMaker.prev}">
		    	<li id="center"><a href="aAllInfo1.do${pageMaker.makeQuery(pageMaker.startPage - 1)}">이전</a></li>
		    </c:if> 
		
		    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
		    	<li id="center"><a href="aAllInfo1.do${pageMaker.makeQuery(idx)}">${idx}</a></li>
		    </c:forEach>
		
		    <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
		    	<li id="center"><a href="aAllInfo1.do${pageMaker.makeQuery(pageMaker.endPage + 1)}">다음</a></li>
		    </c:if> 
	  	</ul>
	</div>
