				...
				<div id="rsm_contents">
					...
					<div class="section_wrap"> <!-- 각 입력 내용 섹션 분류용 래퍼 -->
						<div class="resume_title">
							<input type="hidden" name="res_no" value="${resumeVO.res_no}">
							<input type="text" placeholder="이력서의 제목을 입력해주세요." name="res_title" value="${resumeVO.res_title}">
						</div>
						<h2>기본정보 (필수)</h2>
						<input type="hidden" name="pro_no" value="${resumeVO.pro_no}">
						...
						<div class="occu_wrap"> <!-- 관심 직종 -->
							...
							<div class="occu_select">
								<select id="occu_content" name="pro_occu">
									<option value="none">선택해주세요.</option>
									<option value="IT/인터넷" ${profileVO.pro_occu eq "IT/인터넷" ? "selected" : ""}>IT/인터넷</option>
								</select>
							</div>
							<div class="subOccu_select">
								<select id="subOccu_content" name="pro_subOccu">
									<option value="none">선택해주세요.</option>
									<option value="JAVA" ${profileVO.pro_subOccu eq "JAVA" ? "selected" : ""}>JAVA</option>
									<option value="데이터베이스" ${profileVO.pro_subOccu eq "데이터베이스" ? "selected" : ""}>데이터베이스</option>
									<option value="네트워크" ${profileVO.pro_subOccu eq "네트워크" ? "selected" : ""}>네트워크</option>
									<option value="보안" ${profileVO.pro_subOccu eq "보안" ? "selected" : ""}>보안</option>
								</select>
							</div>
						</div>
						...
						<div class="profile_wrap">
							...
							<div class="proflie1_content">
								<div class="profile_input">
									<div class="set1"> <!-- set1 옆에 프로필 이미지가 라벨 형식으로 붙음 -->
										<input type="text" class="input_name" name="pro_name" placeholder="이름" value="${profileVO.pro_name}">
									</div>
									<div class="set2">
										<input type="text" class="input_email" name="pro_email" placeholder="이메일" value="${profileVO.pro_email}">
									</div>
									<div class="set2">
										<input type="text" class="input_tel" name="pro_tel" placeholder="전화번호" value="${profileVO.pro_tel}">
									</div>
								</div>
								<label class="resume_addPhoto" for="resume_addPhoto_id">
									...
									<span class="resume_addPhoto_img">
										<img style="height:17vh;" src="<c:url value="/resources/images/default_user.png"/>"> <!-- 등록 안 했을시 기본 이미지 path -->
									</span>
								</label>
							</div>
							...
							<div class="profile2_content">
								<div class="flexible_textarea">
									...
									<textarea class="flexible" name="pro_aboutMe" placeholder="경력, 경험 위주의 자기소개를 작성해주세요. 관심분야, 앞으로의 목표등의 내용도 좋습니다.">${profileVO.pro_aboutMe}</textarea>
								</div>
							</div>
						</div>
					</div>
					...
					<div class="section_wrap">
						...
						<input type="hidden" name="e_no" value="${resumeVO.e_no}">
						<c:choose>
						...
						<input type="hidden" name="car_no" value="${resumeVO.car_no}">
						<c:choose>
						...
						<input type="hidden" name="act_no" value="${resumeVO.act_no}">
						<c:choose>
						...
						<input type="hidden" name="cer_no" value="${resumeVO.cer_no}">
						<c:choose>
							...
							<c:otherwise>
								<c:forEach var="certiOtherVO" items="${certiOtherVOList}">
									<div class="certiOthers_body">
										<div class="certiOhtersName">
						...
						<input type="hidden" name="por_no" value="${resumeVO.por_no}">
						<c:choose>
							<c:when test="${portVOList.size() eq 0}">
								<div class="portfolio_body">
									<label class="label_upload_resume">
										<input type="file" class="input_upload_resume" name="por_file">
							...
							<c:otherwise>
								<c:forEach var="portfolioVO" items="${portVOList}">
									<div class="portfolio_body">
										<label class="label_upload_resume">
											<input type="file" class="input_upload_resume" name="por_file" value="${portfolioVO.por_filepath}">
											<span class="btn_upload_resume">파일선택</span>
										</label>
									</div>
								</c:forEach>
